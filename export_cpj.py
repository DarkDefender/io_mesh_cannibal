...
def create_seq_data(obj, armature):
    ...
    for action in bpy.data.actions:
        ...
        if len(action.pose_markers) != 0:
            # TODO: Handle more events besides TRIG and TFLG.
            # Get all markers and convert them to TRIG events
            for marker in action.pose_markers:
                if marker.name.startswith("TFLG"):
                    event_data = ["TFLG", marker.frame / (action.frame_end + 1), action[marker.name]]
                else:
                    event_data = ["TRIG", marker.frame / (action.frame_end + 1), marker.name]
